...
namespace SevenBoldPencil.WeaponCamoAndStickers
{
    ...
    public class MaskOffsetHandle(Plugin plugin, string itemId, int decalIndex, DecalInfo decalInfo, Decal decal, Shader handleShader) : ITransformHandle
    {
        ...
        public void Init(Transform transformHandle, Camera transformHandleCamera, Transform root)
        {
            ...
            var axisX = new GameObject("MaskOffsetAxis.X").AddComponent<PositionAxis>().Initialize(transformHandle, root, Vector3.right, Color.red, _handleShader);
            axisX._handle = positionHandleX;
            var axisZ = new GameObject("MaskOffsetAxis.Z").AddComponent<PositionAxis>().Initialize(transformHandle, root, Vector3.forward, Color.blue, _handleShader);
            axisZ._handle = positionHandleZ;
            var planeXZ = new GameObject("MaskOffsetPlane.XZ").AddComponent<PositionPlane>().Initialize(transformHandle, root, Vector3.right, Vector3.forward, Vector3.up, Color.green, _handleShader);
